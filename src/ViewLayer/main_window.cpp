...
void MainWindow::openFileDialog() {
  ...
  QDBusMessage msg = QDBusMessage::createMethodCall(
      ...
      u"org.freedesktop.portal.FileChooser"_s, u"OpenFile"_s);
  ...
  connect(watcher, &QDBusPendingCallWatcher::finished, this,
          [this](QDBusPendingCallWatcher *w) {
            ...
            QDBusConnection::sessionBus().connect(
                u"org.freedesktop.portal.Desktop"_s, reply.value().path(),
...
void MainWindow::applySS(const QString &resourcePath) {
  if (!m_cssWatcher) {
    ...
    connect(m_cssWatcher, &QFileSystemWatcher::fileChanged, this,
  ...
  QString diskPath;
  for (const QString &c : QStringList{execDir + u'/' + relPath,
                                      execDir + QStringLiteral("/styles/") +
...
void MainWindow::setMargins(const QMargins &m) {
  if (m_pdfScroll)
    m_pdfScroll->setContentsMargins(m);
  if (m_webView)
    m_webView->setContentsMargins(m);
}
