...
public:
  ...
  void enableCasualAutoHide(bool enable);

protected:
  bool eventFilter(QObject *obj, QEvent *event) override;
  void closeEvent(
      QCloseEvent *event) override; // auto-salva no PDF (comportamento Okular)
  void
  resizeEvent(QResizeEvent *event) override; // reposiciona overlay bottomNav
...
private:
  ...
  CasualPdfView *m_casualPdfView =
      nullptr; // spread de 2 páginas (PDF + Casual)
  ...
  bool m_casualAutoHide = false;
  QTimer *m_casualToolbarTimer = nullptr;
  QTimer *m_casualBottomNavTimer = nullptr;
  ...
  void updateBottomNavOffset(); // ajusta espaçador da barra inferior à largura
                                // da sidebar
  ...
  // ── Auto-hide helpers (main_window_scroll.cpp) ────────────────────────
  void onCasualMouseEdge(int localY);
