...
void MainWindow::openDocument(const QString& path)
{
            ...
            this, [this](const QString& p, int pages)
    {
        ...
        if (m_engine->type() == DocumentType::PDF) {
            ...
            if (m_bookTitleLabel) {
                // Exibe título legível: remove extensão e substitui _ / - por espaços
                ...
                // Remove prefixos típicos de filenames de livros: "[Autor]", "(2023)" etc.
                cleanTitle.remove(QRegularExpression(QStringLiteral("^\\[.*?\\]\\s*")));
        ...
        const int startPage = m_sidecar
                              ? std::max(0, m_sidecar->loadPagePosition())
                              : 0;
...
void MainWindow::setSidebarCasualMode(bool casual)
{
    ...
    if (casual && m_engine) {
        ...
        const int pct = (total > 1)
                        ? static_cast<int>((static_cast<double>(current) / (total - 1)) * 100.0)
                        : 0;
...
void MainWindow::wireDocumentSignals()
{
    ...
    connect(m_casualWidget->controller(),
            &CasualModeController::chapterNavigationRequested,
            this,
